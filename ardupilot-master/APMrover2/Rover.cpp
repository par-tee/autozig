 ...
 */
uint16_t weight_info[7]={0};
void Rover::one_second_loop(void)
{
    //golf: weight check  baudrate:9600

	 const uint8_t  require_weight[]={0x05,0xa9,0x00,0x00,0x00,0x0d};//获取重量的命令 05 a9 00 00 00 0d
	 hal.uartE->write(require_weight, 6);
	   ...
	   while(count < 7)
	   	   	{
	   	   		...
	   	   		{
	   	   			if((0==count)&&(0x05!=Data1))
	   	   			{   //count=0;
	   	   				return;
	   	   			}
	   	   		     weight_info[count] = Data1;
	   	             printf("\t %x \t",weight_info[count]);
	   	   			count++;
	   	   		}
	   	   	}
	...
	//golf
	 ...
	 rtc.get_system_clock_utc(hour,min,sec,ms);//获取现在的UTC时间 时 分 秒
     printf("\n hour=%d,min=%d,sec=%d \n",hour,min,sec);

     beijing_start_time_hour=g2.regular_start_time_hour-8;
     beijing_end_time_hour=g2.regular_end_time_hour-8;

     if(beijing_start_time_hour<0)
     {g2.regular_start_time_hour=beijing_start_time_hour+24;}
     if(beijing_end_time_hour<0)
     {g2.regular_start_time_hour=beijing_end_time_hour+24;}

     //if((hour==(int32_t)g2.regular_start_time_hour)&&(min==(int32_t)g2.regular_start_time_min)&&(sec==(int32_t)g2.regular_start_time_sec))//时间到达参数设定值的时候设定小车动作
     if((hour==beijing_start_time_hour)&&(min==(int32_t)g2.regular_start_time_min)&&(sec==(int32_t)g2.regular_start_time_sec))
     {
     rover.set_mode(rover.mode_auto, MODE_REASON_EVERYDAY_START);
     arming.arm(AP_Arming::Method::RUDDER);
     printf("everyday start\n" );
     }

    //if((hour==(int32_t)g2.regular_end_time_hour)&&(min==(int32_t)g2.regular_end_time_min)&&(sec==(int32_t)g2.regular_end_time_sec))
     if((hour==beijing_end_time_hour)&&(min==(int32_t)g2.regular_end_time_min)&&(sec==(int32_t)g2.regular_end_time_sec))
    {
