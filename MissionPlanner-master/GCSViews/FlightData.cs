...
// square
        ...
        /*****Marko Jajalo, mjajalo4@gmail.com, 
        the code I wrote is all the stuff below this comment*****/

        //AutoHome
        //Sends the rover back to the launch site
        private void myButton4_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                MainV2.comPort.doCommand((MAVLink.MAV_CMD)Enum.Parse(typeof(MAVLink.MAV_CMD), "RETURN_TO_LAUNCH"),
                    param1, 0, param3, 0, 0, 0, 0);
            }
            catch
            {
        ...
        //Scheduel Mission button
        //Which will send messages to the rover periodically using
        //the function runAutoAt()
        int clicked = 0;
        private void myButton5_Click(object sender, EventArgs e)
        {
            clicked++;
            int startHour = (int)startNumHour.Value;
            int startMinutes = (int)startNumMin.Value;
            int endHour = (int)endNumHour.Value;
            int endMinutes = (int)endNumMin.Value;

            var dateNow = DateTime.Now;
            if (startHour < 0 || startHour > 23 || startMinutes < 0 || startMinutes > 59 ||
                endHour < 0 || endHour > 23 || endMinutes < 0 || endMinutes > 59)
            {
                CustomMessageBox.Show("Starting on time has failed. \nValid hours: 0-23\nValid minutes: 0-59");
            }
            else {
                var date = new DateTime(dateNow.Year, dateNow.Month, dateNow.Day, startHour, startMinutes, 0);
                var endDate = new DateTime(dateNow.Year, dateNow.Month, dateNow.Day, endHour, endMinutes, 0);
                var tokenSource = new CancellationTokenSource();
                runAutoAt(date, endDate);              
            }
            
        }

        //Creates worker threads to send a MAVLink message to the rover
        //depending on the start and end time
        private void runAutoAt(DateTime date, DateTime endDate)
        {
            ...
            Task.Delay(ts).ContinueWith((x) => someMethod(0), cts.Token);
            if (endDate > dateNow)
            {
                ts = endDate - dateNow;
            }
            ...
            Task.Delay(ts).ContinueWith((x) => someMethod(1), cts.Token);
            if (clicked > 1)
            {
                cts.Cancel();
                clicked = 0;
        ...
        //This method actually sends the MAVlink message to either start the mission
        //or return the rover back to the launch site.
        private void someMethod(int end)
        {
            //try
            //{
                //((Button)sender).Enabled = false;
        ...
        //Scheduel on Framework button
        //The framework needs to be modified to store a start and end time.
        //Both start time and end time are found as an offset of seconds, 
        //ie) "how many seconds from now should the rover start and end the mission?".
        //The framework has a scheduler (TASK_SCHED in APMrover2.cpp of APMrover2 framework version 3.4.2) 
        //that needs to be modified to accept the 2 parameters down below (param4start and param5end).
        //The framework can be downloaded via github and compiled on a linux system by using the terminal to go to:
        //ardupilot->APMrover2, once in here you can type make px4-v3. I have determined the rover's cpu is px4-v3 using
        //mission planner. I have tried using threads within the framework but it didn't work so try and modify the TASK_SCHED component
        //to sample the time every few Hz and if it has passed the required startInSeconds value then trigger the MISSION_START MAVlink command.
        //After the endInSeconds has been reached (by the rover's internal clock) the RETURN_TO_LAUNCH MAVlink command should be triggered.
        private void myButton6_Click(object sender, EventArgs e)
        {
            ...
            {
                int startInSeconds = Math.Abs(((startHour * 3600) + (startMinutes * 60)) - ((dateNow.Hour * 3600) + (dateNow.Minute * 60)));
                int endInSeconds = Math.Abs(((endHour * 3600) + (endMinutes * 60)) - ((dateNow.Hour * 3600) + (dateNow.Minute * 60)));
                CustomMessageBox.Show($"Program will start in {(startInSeconds/60)} minutes, and end in {endInSeconds/60} minutes\n");

                //this part should be parsed by the framework and should use param4Start, and param5End to determine when to start mission
                //end when to return to launch.
                int param1 = 0; //param1 and param3 for some reason must be these values.
                int param3 = 1;
                int param4Start = startInSeconds;
                int param5End = endInSeconds;
                
                //send the command below via MAVLink protocol
                /*MainV2.comPort.doCommand((MAVLink.MAV_CMD)Enum.Parse(typeof(MAVLink.MAV_CMD), "MISSION_START"),
                param1, 0, param3, param4Start, param5End, 0, 0);*/
            }
        }
    }
}
