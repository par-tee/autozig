...
Column 1: Field type (RALLY is the only one at the moment -- may have RALLY_LAND in the future)
 ...
 Column 7: Flags (just 0 for now, also future use).
        ...
        //Marko Jajalo, mjajalo4@gmail.com wrote the code below
        //This button reverses the waypoints on the flightPlanner view.
        private void myButton1_Click(object sender, EventArgs e)    //reverses the waypoints
        {
            int startIndex = Commands.Rows.Count;      //begin with the last waypoint added
            for (int a = startIndex - 2; a >= 0; a--)   //add waypoints in reverse order
            {
                DataGridViewRow myrow = (DataGridViewRow)Commands.Rows[a].Clone();
                for (int b = 0; b < myrow.Cells.Count; b++)
                {
                    myrow.Cells[b].Value = Commands.Rows[a].Cells[b].Value;
                }
                Commands.Rows.Insert(startIndex, myrow);
                startIndex++;
            }
            writeKML();
            //CustomMessageBox.Show(DateTime.Now.ToString());
        }
    }
}